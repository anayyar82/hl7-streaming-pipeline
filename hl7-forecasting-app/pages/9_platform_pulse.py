...
from utils.db import run_query, run_query_batch
...
def _platform_pulse_body() -> None:
    # ---- Live KPIs ----
    st.header("Live snapshot")
    
    c1, c2, c3, c4 = st.columns(4)

    pulse_batch = run_query_batch(
        {
    ...
    enc = pulse_batch.get("enc", pd.DataFrame())
    try:
        n_enc = int(enc["n"].iloc[0]) if not enc.empty else 0
    except Exception:
        n_enc = 0

    ml = pulse_batch.get("ml", pd.DataFrame())
    try:
        if not ml.empty:
    ...
    msg = pulse_batch.get("msg", pd.DataFrame())
    try:
        n_msg = int(msg["messages_24h"].iloc[0]) if not msg.empty else 0
    except Exception:
        n_msg = 0

    pat = pulse_batch.get("pat", pd.DataFrame())
    try:
...
run_live_dashboard(_platform_pulse_body, interval_seconds=22, manual_key="hl7_platform_pulse_refresh")
