...
from utils.filters import (
    ...
    read_multiselect_subset_or_all,
)
...
def _operations_sidebar() -> None:
    """All ``st.sidebar`` widgets — must run outside ``st.fragment``."""
    ...
    if not msg_df.empty:
        ...
        st.sidebar.multiselect("Message Type", msg_types, default=msg_types, key="ops_mtype")
        facilities = sorted(msg_df["sending_facility"].dropna().unique())
        st.sidebar.multiselect("Sending Facility", facilities, default=facilities, key="ops_sfac")
    ...
    if not activity_df.empty:
        ...
        date_range_filter(activity_df, key="ops_pa_dr", label="Activity Date Range")
...
def _operations_main() -> None:
    ...
    try:
        ...
        if throughput_df.empty:
            ...
            tp_start, tp_end = read_date_range_selection(
                throughput_df, col="processing_hour", key="ops_tp_dr",
            )
            ...
            if throughput_df.empty:
                ...
                fig.add_trace(
                    go.Bar(
                        x=throughput_df["processing_hour"],
    ...
    try:
        ...
        if msg_df.empty:
            ...
            msg_types = sorted(msg_df["message_type"].dropna().unique())
            sel_msg_types = read_multiselect_subset_or_all(msg_types, "ops_mtype")
            facilities = sorted(msg_df["sending_facility"].dropna().unique())
            sel_facilities = read_multiselect_subset_or_all(facilities, "ops_sfac")
    ...
    try:
        ...
        if activity_df.empty:
            ...
            fac_sel = read_facility_selection(activity_df, key="ops_fac")
            activity_df = apply_facility(activity_df, fac_sel)

            pa_start, pa_end = read_date_range_selection(activity_df, key="ops_pa_dr")
            activity_df = apply_date_range(activity_df, pa_start, pa_end)

            patient_classes = sorted(activity_df["patient_class_desc"].dropna().unique())
            sel_classes = read_multiselect_subset_or_all(patient_classes, "ops_pclass")
            if sel_classes:
            ...
            if activity_df.empty:
                ...
                fig.update_layout(height=400, margin=dict(t=40), legend=dict(orientation="h", y=1.1))
                st.plotly_chart(fig, use_container_width=True)
                ...
                with col_b:
                    ...
                    fig_loc = px.pie(
                        location_agg,
                        names="location_facility",
                        values="total_encounters",
                        title="Encounters by Facility",
                        hole=0.4,
                    )
...
run_live_dashboard(
    _operations_main,
    interval_seconds=22,
    manual_key="hl7_operations_live_refresh",
    before_fragment=_operations_sidebar,
)
